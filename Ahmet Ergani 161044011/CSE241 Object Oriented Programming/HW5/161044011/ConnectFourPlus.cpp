...
int ConnectFourPlus::initBoard()
{
    board = new Cell *[height];         //allocating
    for (int i = 0; i < height; i++)
        board[i] = new Cell[width];
    for (int i = 0; i < height; i++)
        for (int j = 0; j < width; ++j)
        {
            board[i][j].setRow(i);      //assigning the cells
            board[i][j].setColumn(j);
        }
    totalDot = height * width;          //total cells
    return 1;
}
...
int ConnectFourPlus::horizontal()
{
